...
class SeaIce:
    ...
    def __init__(self,
        ...
        if mode == "MY":
            raise ValueError("Multiyear not yet implemented. Should not be too hard to add")            
    ...
    def get(self, d: date, lat: np.ndarray, lon: np.ndarray) -> xr.Dataset:
        ...
        for cfg in self.ftp_configs:
            ...
            ftp_connexion = cfg['ftp'] # get ftp connexion
            
            # check if both files exists
            ftp_n_check = ftp.ftp_file_exists(ftp_connexion, ftp_n) # check if file north exists 
            ftp_s_check = ftp.ftp_file_exists(ftp_connexion, ftp_s) # check if file south exists
            ...
            if (ftp_n_check and ftp_s_check):
                try:
                    ...
                    ftp.ftp_download(ftp_connexion, self.directory / ftp_n.name, ftp_n.parent)
                    ftp.ftp_download(ftp_connexion, self.directory / ftp_s.name, ftp_s.parent)
                except: 
                    raise
